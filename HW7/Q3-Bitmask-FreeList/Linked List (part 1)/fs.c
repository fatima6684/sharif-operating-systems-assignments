...
bool load_superblock(superblock_t *sb) {
    ...
    bool result = (n == sizeof(superblock_t));
    if (!result)
        printf("Failed to load_superblock!\n");
    return result;
}
...
bool save_superblock(const superblock_t *sb) {
    ...
    bool result = (n == sizeof(superblock_t));
    if (!result)
        printf("Failed to save_superblock!\n");
    return result;
}
...
bool create_fs(const char *path) {
    ...
    save_free_entry(&initial_free);

    // 4. Set file size: write a '\0' at the last byte of the fs to set its size
...
bool load_file_entry(uint32_t offset, file_entry_t *ent) {
    ...
    // bool result = (n == sizeof(file_entry_t));
    // if (!result)
    //     printf("Failed to load_file_entry properly!\n");
    // return result;
...
void save_file_entry(uint32_t offset, const file_entry_t *ent) {
    ...
    fflush(g_fs_file);
    if (n != sizeof(file_entry_t))
        printf("Failed to save_file_entry properly!\n");
}

uint32_t find_file(const char *name, file_entry_t *out) { // attaches file_entry, returns offset
    // if (!out) {
    //     printf("find_file()'s second argument is invalid!\n");
    //     return 0;
    // }
    superblock_t sb;
    load_superblock(&sb);
    ...
    while (cur != 0) {
        ...
        if (!load_file_entry(cur, &ent)) {
            printf("load_file_entry failed! inconsistent file_entries!\n");
            return 0;
        }
...
uint32_t create_file(const char *name) {
    ...
    if (off == 0) {
        printf("Failed to allocate space to file entry!\n");
        return 0;
    }
...
void file_stats() {
    ...
    if (!load_file_entry(g_open_file_offset, &ent)) {
        printf("Error: cannot load file entry.\n");
        return;
    }
...
void fs_stats() {
    ...
    printf("  SuperBlock size: %u bytes (%.2f KB)\n", SUPERBLOCK_SIZE, SUPERBLOCK_SIZE / 1024.0);
    printf("  File count: %u\n", sb.file_count);
...
void write_file(uint32_t pos, uint32_t nbytes) {
    ...
    uint32_t new_size = pos + nbytes;
    // Allocate initial block if empty
    if (ent.data_offset == 0) { // file is empty
        ent.data_offset = fs_alloc(new_size);
        if (ent.data_offset == 0) {
            printf("Failed to allocate space to file, in order to write in it!\n");
            return;
        }
        ent.size = new_size; // Set initial size
    } else if (ent.size == 0) {
        perror("this file's size is zero, but its data_offset is not zero!");
        return;
    } else if (ent.size < new_size) { // file is smaller than new_size
        // 1. Read old file data
        char *old_data = NULL;
        old_data = malloc(ent.size);
        if (!old_data) {
            printf("Memory allocation failed.\n");
            return;
        }
        fseek(g_fs_file, ent.data_offset, SEEK_SET);
        fread(old_data, 1, ent.size, g_fs_file);
        
        // 2. Free old data region
        fs_free(ent.data_offset, ent.data_offset + ent.size);

        // 3. Allocate new space
        uint32_t new_offset = fs_alloc(new_size);
        if (new_offset == 0) {
            printf("Failed to allocate space to file, in order to write in it!\n");
            free(old_data);
            return;
        ...
        // 4. Write old data back to new location
        fseek(g_fs_file, new_offset, SEEK_SET);
        fwrite(old_data, 1, ent.size, g_fs_file);
        free(old_data);
            
        ent.data_offset = new_offset;
        ent.size = new_size;
    }
    ...
    if (buffer) {
        ...
        int c;  /* consume the trailing newline if present */
        do {
            c = getchar();
        } while (c != '\n' && c != EOF);

        fseek(g_fs_file, ent.data_offset + pos, SEEK_SET);
...
void read_file(uint32_t pos, uint32_t nbytes) {
    ...
    if (pos >= ent.size) {
        printf("Chosen position(%d) >= file size(%d)!\n", pos, ent.size);
        return;
    }
...
void shrink_file(uint32_t newsize) {
    ...
    uint32_t old_end = ent.data_offset + ent.size;
    uint32_t new_end = ent.data_offset + newsize;
...
void remove_file() {
    ...
    file_entry_t ent;
    if (!load_file_entry(g_open_file_offset, &ent)) {
        perror("failed to load_file_entry!");
    }
    ...
    // 2. Free Data
    if (ent.data_offset != 0 && ent.size > 0) {
        fs_free(ent.data_offset, ent.data_offset + ent.size);
    }
...
bool load_free_entry(uint32_t offset, free_entry_t *ent) {
    fseek(g_fs_file, offset, SEEK_SET);
    size_t n = fread(ent, 1, sizeof(free_entry_t), g_fs_file);
    // bool result = (n == sizeof(free_entry_t));
    // if (!result)
    //     printf("Failed to load_free_entry properly!\n");
    // return result;
    return (n == sizeof(free_entry_t));
}

bool save_free_entry(const free_entry_t *ent) {
    fseek(g_fs_file, ent->start, SEEK_SET);
    size_t n = fwrite(ent, 1, sizeof(free_entry_t), g_fs_file);
    fflush(g_fs_file);
    bool result = (n == sizeof(free_entry_t));
    if (!result)
        printf("Failed to save_free_entry!\n");
    return result;
}
...
void visualize_freelist() {
    ...
    // not needed, considering free entries in disk are in order
...
uint32_t fs_alloc(uint32_t size) { // returns starting offset
    if (size == 0) return 0;
    ...
    while (curr_ptr != 0) {
        ...
        if (available >= size) {
            ...
            if (prev_ptr == 0) {
                ...
                prev.next = next_node;
                save_free_entry(&prev);
            }

            // Zero out allocated region
            if (fseek(g_fs_file, alloc_start, SEEK_SET) != 0) {
                perror("fseek");
                return 0;
            }
            const size_t ZERO_BUF_SIZE = 4096;
            static uint8_t zero_buf[4096] = {0};
            uint32_t remains = size;
            while (remains > 0) {
                size_t chunk = remains > ZERO_BUF_SIZE ? ZERO_BUF_SIZE : remains;
                if (fwrite(zero_buf, 1, chunk, g_fs_file) != chunk) {
                    perror("fwrite");
                    return 0;
                }
                remains -= chunk;
            ...
            // return offset of the begining of the allocated space
            return alloc_start;
        }
    ...
    printf("Error: No contiguous free space of size %u found.\n", size);
    return 0;
}
