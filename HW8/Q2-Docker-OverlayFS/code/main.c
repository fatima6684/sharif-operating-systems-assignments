...
int main(int argc, char **argv) {
  ...
  while (i < argc) {
    if (strcmp(argv[i], "run") == 0) {
      ...
      if (i + 1 >= argc) {
        fprintf(stderr,
                "[ERR] Missing --base-dir value (e.g. [--base-dir "
      ...
      if (i + 1 >= argc) {
        fprintf(stderr,
                "[ERR] Missing --base-image value (e.g. [--base-image "
                "docker.arvancloud.ir/python:3.11]).\n");
        return 1;
      }
      strncpy(cfg.base_image, argv[++i], sizeof(cfg.base_image) - 1);
      cfg.base_image[sizeof(cfg.base_image) - 1] = '\0';
      i++;
    } else {
      // باقی‌مانده را به عنوان command در نظر می‌گیریم (مثل نسخه‌ی خودت)
